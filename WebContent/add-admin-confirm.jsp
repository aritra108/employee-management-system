	...
	<body>
		...
		<div class="main-section add-admin">
			...
			<%
				...
				if(Driver.validateAdminID(id)) {
					int rows = Driver.addAdmin(id, name, designation, password);
					out.println("<h3 class='message'>The admin has successfully been added!</h3>");
				} else {
					out.println("<h3 class='message'>The entered ID is already assigned to another admin!</h3>");
				}
			%>
		</div>
		...
		<script>
			...
			form.addEventListener("submit", e => {
				...
				if(password.value.length < 4 || password.value.length > 20) {
					alert("Password must be at least 6 characters and atmost 20 characters long!")
					return;
				}
